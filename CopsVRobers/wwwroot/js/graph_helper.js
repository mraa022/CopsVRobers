...
function mainLoop(player,cy,playerId,playerSocket){ // player is a string denoting which player is playing the game (cop or robber)
    // var cy = createGraph();
    var prev_cop = null;
    ...
    // This eventlistener is triggered when the other player makes a move.
    playerSocket.addEventListener("message", (event) => {
        var receivedMessage = JSON.parse(event.data);
        if (receivedMessage.Type=="move"){
            var nodeId = receivedMessage.nodeId  // the id of the node the other player clicked on
            var clickedNode = null
            cy.nodes().forEach(function (Node) {
                console.log("clicked id: " + nodeId + " curr node Id: "+ Node.id())
                if(Node.id()==nodeId){
                    clickedNode= Node;
            ...
            if (curr_player=='cop'){
                if (prev_cop && clickedNode.id() == prev_cop.id()){ // game over
                    alert("game over. Cop won");
                    reset(cy);
                    curr_player = "cop"
                    prev_cop= null;
    ...
    cy.on('click', 'node', function(event) {
        ...
        // cop's turn
        if (curr_player == "cop" && player=="cop"){ // if its cop's turn and cop makes a move
        ...
        // robber's turn
        else if (curr_player == "robber" && player=="robber"){
